...
if(isset($_POST['submit'])){
    ...
    $sql = "UPDATE `athlete` SET `Title`='$Title',`Name`='$Name',`Lastname`='$Lastname',`Department`='$Department',`Color`='$Color',`Sport`='$Sport',`Time`='$Time',`Date`='$Date',`Place`='$Place' WHERE id=$id";

    $result = mysqli_query($conn, $sql);

    if($result){
        header("Location: athletea.php?msg=Update Athele have save!");
    }
    else {
...
<body>
    <header>
        ...
        <?php
        $sql = "SELECT * FROM `athlete` WHERE id = $id LIMIT 1";
        $result = mysqli_query($conn, $sql);
